...
def analyze_multi_rank(step_trace_results):
    ...
    # Identify slow ranks (> mean + 5%)
    threshold = mean_total * 1.05
    slow_ranks = [r["device_id"] for r in ranks if r["total_us"] > threshold]
...
def print_json_report(op_results, step_results, comm_results, comm_shape_results=None, multi_rank=None):
    """输出 JSON 格式报告。"""
    report = {
...
def main():
    ...
    # Multi-rank analysis
    multi_rank = analyze_multi_rank(step_results)

    # Output
    if args.json:
        print_json_report(op_results, step_results, comm_results, comm_shape_results, multi_rank)
    else:
