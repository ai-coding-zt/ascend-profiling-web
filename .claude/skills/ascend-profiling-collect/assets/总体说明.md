<!-- source: https://www.hiascend.com/document/detail/zh/canncommercial/850/devaids/Profiling/atlasprofiling_16_0057.html -->
...
*表示{timestamp}时间戳。
  ...
  * device__{id}_ 目录主要保存各个Device运行昇腾AI应用的性能原始数据和昇腾AI处理器系统原始数据。
  * host目录主要保存上层应用接口（msproftx）的昇腾AI应用运行性能原始数据和Host系统原始数据。


默认采集的性能数据文件如表1所示。

**表1** msprof默认配置采集的性能数据文件文件名 | 说明
